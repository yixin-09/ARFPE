...
namespace {
   ...
   */
  struct FPUnderflowPass : public ModulePass {
  public:
    static char ID;

    FPUnderflowPass() : ModulePass(ID) {
    }
    ...
    int checkFunction(Function &currFunc,
        ...
        Function &gl_fpOfFun, GlobalVariable &gl_stflag,
...
//      if(w_ids==1){
        dbgs() << "[+] Insert Underflow analysis code in function:" << funcName << "\n";
        dbgs() << "[-] w_ids:" << w_ids << "\n";
//      }
            ...
            for(auto &currIns: currBB) {
              ...
              if (CallInst *callInst = dyn_cast<CallInst>(&currIns)) {
                ...
                if(!(callFunc -> isDeclaration())) {
                    if(CallfuncName.compare("fpUnderflow")!=0){
...
//                        dbgs() << "[-] call_size:" << call_size2 << "\n";
                        if((stop_call_flag!=1)&&(call_size2<5)){
                            cnt_idx = checkFunction(*callFunc, w_ids, gl_w, gl_idx, gl_cidx, gl_fpOfFun,gl_stflag,callsFuncs2,call_size2);
...
//                            call_size = call_size2 - 1;
              ...
              if (isFPOperation(inst)||isCallFPOperation(inst)) {
...
//                Value* scnt = builder.CreateLoad(IntType,cnt);&gl_idx,
                ...
                builder.CreateCall(&gl_fpOfFun, ArrayRef< Value* > {one,&gl_w,&gl_cidx,returnval,&gl_stflag});
...
//                SplitBlockAndInsertIfThen(cond,ret_stat,true);
    ...
    bool runOnModule(Module &m) override {
      ...
      Function *gl_fpOfFun = m.getFunction("fpUnderflow");
      ...
      for (auto &currFunc: m) {
        ...
        if(!currFunc.isDeclaration()) {
            if(funcName.compare(InputFilename)==0){
                w_ids = checkFunction(currFunc, w_ids, *gl_w, *gl_idx, *gl_cidx, *gl_fpOfFun,*gl_stflag,callsFuncs,call_size);
            }
        }
      }
  ...
  char FPUnderflowPass::ID = 0;
  // pass arg, pass desc, cfg_only, analysis only
  static RegisterPass<FPUnderflowPass> x("Underflow",
                                              "Insert Underflow check for each fp instruction.",
                                                    true,
                                                    true);
}
