...
namespace {
   ...
   */
  struct FPOverflowPass : public ModulePass {
  public:
    static char ID;

    FPOverflowPass() : ModulePass(ID) {
    }
...
//      if(w_ids==1){
        dbgs() << "[+] Insert overflow analysis code in function:" << funcName << "\n";
        dbgs() << "[-] w_ids:" << w_ids << "\n";
//      }
            ...
            for(auto &currIns: currBB) {
              ...
              if (CallInst *callInst = dyn_cast<CallInst>(&currIns)) {
                ...
                if(!(callFunc -> isDeclaration())) {
                    if(CallfuncName.compare("fpOverFlow")!=0){
...
//                SplitBlockAndInsertIfThen(cond,ret_stat,true);
    ...
    bool runOnModule(Module &m) override {
      ...
      Function *gl_fpOfFun = m.getFunction("fpOverFlow");
  ...
  char FPOverflowPass::ID = 0;
  // pass arg, pass desc, cfg_only, analysis only
  static RegisterPass<FPOverflowPass> x("overflow",
                                              "Insert overflow check for each fp instruction.",
                                                    true,
                                                    true);
}
